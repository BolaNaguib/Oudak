<?php include('header.php'); ?>
<section>
  <div class="uk-container ">
    <ul class="uk-breadcrumb">

    <li class="uk-disabled"><a href="#">Product</a></li>
<li><span>Shopping Bag</span></li>
</ul>
    <div class="uk-flex uk-flex-middle" uk-grid>
      <div class="uk-width-auto">
        <ul class="uk-iconnav uk-iconnav-vertical">
          <li>
            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <span uk-icon="icon: heart"></span>
            </button>
          </li>
          <li>

            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <span uk-icon="icon: star"></span>
            </button>
          </li>
          <li>

            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <span uk-icon="icon: mail"></span>
            </button>
          </li>
          <li>
            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <span uk-icon="icon: social"></span>
            </button>
          </li>

        </ul>
      </div>
      <div class="uk-width-expand">
      <div class="uk-card uk-card-default uk-padding-small">
        <div class="" uk-grid>
          <div class="uk-width-1-3">
            <div class="card uk-padding-small">
              <img src="img/product.png" alt="">
              <b>Description</b>
              <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit</p>
            </div>


          </div>
          <div class="uk-width-2-3  uk-padding-small ">
            <h2>Perfume bottle</h2>
            <div class="card uk-margin uk-padding-small">
                <div class="uk-clear-fix">
                  <b class="uk-text-left">Size : </b>
                  <span class="uk-text-right">L</span>
                </div>
            </div>
            <div class="card uk-margin uk-padding-small">
                <div class="uk-clear-fix">
                  <b class="uk-text-left">Price : </b>
                  <span class="uk-text-right">99 $</span>
                </div>
              </div>

            <div class="card uk-margin uk-padding-small">
              <div class="uk-clear-fix">
                <b class="uk-text-left">Quantity : </b>
                <span class="uk-text-right">  <input type="number" placeholder="1" name="" step="1" style="
      text-align: center;
      border: 1px solid #eee;
  "></span>
              </div>
            </div>

    <button class="uk-button uk-button-default  uk-width-expand" type="submit"><span uk-icon="trash"></span>
 Remove </button>
          </div>
        </div>
      </div>



        </div>

      <div class="uk-width-auto">
        <ul class="uk-iconnav uk-iconnav-vertical">
          <li>
            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <b>Total Price</b> <span>99 $</span>
            </button>
          </li>


        </ul>
      </div>
    </div>
    <hr>
    <div class="uk-flex uk-flex-middle" uk-grid>
      <div class="uk-width-auto">
        <ul class="uk-iconnav uk-iconnav-vertical">
          <li>
            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <span uk-icon="icon: heart"></span>
            </button>
          </li>
          <li>

            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <span uk-icon="icon: star"></span>
            </button>
          </li>
          <li>

            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <span uk-icon="icon: mail"></span>
            </button>
          </li>
          <li>
            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <span uk-icon="icon: social"></span>
            </button>
          </li>

        </ul>
      </div>
      <div class="uk-width-expand">
      <div class="uk-card uk-card-default uk-padding-small">
        <div class="" uk-grid>
          <div class="uk-width-1-3">
            <div class="card uk-padding-small">
              <img src="img/product.png" alt="">
              <b>Description</b>
              <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit</p>
            </div>


          </div>
          <div class="uk-width-2-3  uk-padding-small ">
            <h2>Perfume bottle</h2>
            <div class="card uk-margin uk-padding-small">
                <div class="uk-clear-fix">
                  <b class="uk-text-left">Size : </b>
                  <span class="uk-text-right">L</span>
                </div>
            </div>
            <div class="card uk-margin uk-padding-small">
                <div class="uk-clear-fix">
                  <b class="uk-text-left">Price : </b>
                  <span class="uk-text-right">99 $</span>
                </div>
              </div>

            <div class="card uk-margin uk-padding-small">
              <div class="uk-clear-fix">
                <b class="uk-text-left">Quantity : </b>
                <span class="uk-text-right">  <input type="number" placeholder="1" name="" step="1" style="
      text-align: center;
      border: 1px solid #eee;
  "></span>
              </div>
            </div>

    <button class="uk-button uk-button-default  uk-width-expand" type="submit"><span uk-icon="trash"></span>
 Remove </button>
          </div>
        </div>
      </div>



        </div>

      <div class="uk-width-auto">
        <ul class="uk-iconnav uk-iconnav-vertical">
          <li>
            <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
              <b>Total Price</b> <span>99 $</span>
            </button>
          </li>


        </ul>
      </div>
    </div>

<div class="" uk-grid>
  <div class="uk-width-1-2">

  </div>
  <div class="uk-width-1-2">
<div class="" uk-grid>
  <div class="uk-width-1-3">

  </div>
  <div class="uk-width-2-3">
    <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
      <b>Total Price</b> <span>99 $</span>
    </button>
  </div>
  <div class="uk-width-1-3">
    Promo Code
  </div>
  <div class="uk-width-2-3">
  <button class="uk-button uk-button-default uk-width-expand" type="button" uk-toggle="target: #addcoupon">Add Coupon</button>
  <div id="addcoupon" hidden>
    <div class="uk-display-block">
      <form class="uk-search uk-search-default">
        <input class="uk-input" type="search" placeholder="ex : EFGXXDS">
      </form>
    </div>

  </div>
  </div>
  <div class="uk-width-1-3">
    Total Price
  </div>
  <div class="uk-width-2-3">
    <button type="button" name="button" class="uk-button uk-button-default uk-width-expand">
      <b>Total Price</b> <span>99 $</span>
    </button>
  </div>

</div>
    <hr>

                  <input type="checkbox">   <b> I have been able to read and understant <a href="#"> Privacy policy . </a></b><br>
                  <input type="checkbox">   <b> I want to subscribe to the news letter . </b>
                  <hr>
                  <button class="uk-button uk-button-default  uk-width-expand" type="submit"><span uk-icon="check"></span>
                Checkout </button>

  </div>
</div>

  </div> <!-- END .uk-container -->



</section>
<?php include('footer.php'); ?>
